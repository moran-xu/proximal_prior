# -*- coding: utf-8 -*-
"""
Created on Mon Jul 19 22:09:05 2021
...
ll_upp = torch.quantile(ll,.95,0).detach().numpy()
ll_low = torch.quantile(ll,.05,0).detach().numpy()
...
for i in range(3):
    ...
    plt.fill_between(range(25),ll_low[i],ll_upp[i])
    plt.xlabel('t')
plt.legend()
